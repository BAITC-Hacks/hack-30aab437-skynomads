...
- [x] Authorized release: current intended work pushed to main, release candidate `757430d` (submission preparation in `fb1a7b0`). Pre-deploy checks passed: 12 tests, local Node/Worker smoke and Worker dry-run.
- [ ] Deploy Cloudflare Worker after target account/authentication. ELDOC explicitly excluded; Wrangler still unauthenticated. See `history/release-cloudflare.md`.
